...
public class SubDomainVisitCount {
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for(String dString : cpdomains){
            String[] splitStr = dString.split(" ");
            String visitCnt = splitStr[0];
            String domainStr = splitStr[1];

            
            //domainMap.put(domainStr, Integer.parseInt(visitCnt));

            while(true){
             
                if(domainMap.containsKey(domainStr)){
                    Integer cnt = domainMap.get(domainStr);
                    cnt = cnt + Integer.parseInt(visitCnt);
                    domainMap.put(domainStr, cnt);
                }else{
                    domainMap.put(domainStr, Integer.parseInt(visitCnt));
                }
        ...
        for(String key : domainMap.keySet()){
            Integer cnt = domainMap.get(key);
            retList.add(cnt.toString() + " " + key );
        }
        return retList;
    }
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(svc.subdomainVisits(arr));
